...
const ensureProject = (data: Partial<ProjectData>): ProjectData => {
  ...
  const fallbackPage =
    pages.length > 0 ? pages : [{ id: createId(), name: "Page 1", nodes: [] }];
  const activePageId =
    data.activePageId &&
    fallbackPage.some((page) => page.id === data.activePageId)
      ? data.activePageId
      : fallbackPage[0].id;
...
export default function useProjectState() {
  ...
  const activePage = useMemo(() => {
    ...
    return (
      project.pages.find((page) => page.id === project.activePageId) ?? null
    );
  }, [project]);
  ...
  const handleRemovePage = useCallback(
    (pageId: string) => {
      updateProject((prev) => {
        ...
        const nextActive = nextPages.some(
          (page) => page.id === prev.activePageId,
        )
          ? prev.activePageId
          : nextPages[0].id;
        return {
